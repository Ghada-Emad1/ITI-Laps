...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($password) {
            $stmt = $connection->prepare("UPDATE users SET first_name = ?, last_name = ?, address = ?, country = ?, gender = ?, username = ?, password = ?,img=? WHERE id = ?");
            $stmt->execute([$fname, $lname, $address, $country, $gender, $username, $password, $img, $userId]);
        } else {
            $stmt = $connection->prepare("UPDATE users SET first_name = ?, last_name = ?, address = ?, country = ?, gender = ?, username = ?,img=? WHERE id = ?");
            $stmt->execute([$fname, $lname, $address, $country, $gender, $username, $img, $userId]);
        }
